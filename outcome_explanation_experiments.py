...
for (dataset_name, sample_size) in [("breast", 30), ("compas", 100), ("bank", 100), ("adult", 100)]:
    # configure paths
    ...
    for i in sample_df.index:
        ...
        try: # sometimes (e.g. for the bank dataset) we are not able obtain valid result from anchors
            anchor_rule_support = len(df.query(" & ".join(anchor_rule.names()), engine='python'))
            anchor_rule_positive_support = len(df.query(" & ".join(anchor_rule.names()), engine='python').query('y_pred > 0'))
        ...
        te2_rule_positive_support = len(df.query(" & ".join(te2_rule), engine='python').query('y_pred > 0'))
    ...
    print("Average runtime per instance for te2rules: ", te2rules_result.computation_time.mean())
    print("Average runtime per instance for anchors: ", anchors_result.computation_time.mean())

    # write summary result to txt
    f = open(output_path + "/summary_stats.txt", "w")
    f = open(output_path + "/summary_stats.txt", "a")
    f.write("\nAverage runtime per instance for te2rules: {:.4f}\n".format(te2rules_result.computation_time.mean()))
    f.write("Average runtime per instance for anchors:  {:.4f}\n".format(anchors_result.computation_time.mean()))
    f.close()
