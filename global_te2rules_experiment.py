...
def experiment(dataset_name_params, n_estimators_params, max_depth_params, num_stages_params):
    dataset_col = []
    n_estimators_col= []
    max_depth_col=[]
    num_stages_col=[]
        ...
        for n_estimators in n_estimators_params:
            for max_depth in max_depth_params:
                for num_stages in (num_stages_params + [n_estimators]):
                    print("experiment: {} data, {} trees, {} depth, {} stages".format(dataset_name, n_estimators, max_depth, num_stages))
                    accuracy, auc, num_rules, fidelities, total_time = experiment_iter(dataset_name, n_estimators, max_depth, num_stages)
                    print()
...
def experiment_iter(dataset_name, n_estimators, max_depth, num_stages):    
    ...
    # write to summary
    with open(os.path.join(output_path, "summary_t{}_d{}_s{}.txt".format(n_estimators, max_depth, num_stages)), 'w') as f:
    ...
    # write to rules
    with open(os.path.join(output_path, "rules_t{}_d{}_s{}.txt".format(n_estimators, max_depth, num_stages)), 'w') as f:
        for r in rules:
            f.write(str(r) + '\n')

    return accuracy, auc, len(rules), fidelities, total_time

experiment(dataset_name_params, n_estimators_params, max_depth_params, num_stages_params)
